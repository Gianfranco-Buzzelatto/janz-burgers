...
function NewProductWizard({ ingredients, config, onClose, onSaved }) {
  ...
  const [prices, setPrices] = useState({ x1: '', x2: '', x3: '' });
  ...
  const handleSave = async () => {
    ...
    if (!prices.x1 || !prices.x2 || !prices.x3) { toast.error('Los precios de venta son obligatorios'); return; }

    setSaving(true);
    try {
      // Crear receta y producto para cada variante
      const validRows = recipeRows.filter(r => r.ingredient && r.quantity);
      const multipliers = { x1: 1, x2: 2, x3: 3 };
      ...
      <div className="modal" style={{ maxWidth: 580 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          {wizardStep === 2 && (
            <>
              <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--gray)', textTransform: 'uppercase', marginBottom: 16 }}>
                Paso 2 — Receta base (x1 medallón)
              </div>
              <div style={{ color: 'var(--gray)', fontSize: '0.8rem', marginBottom: 16 }}>
                Cargá los ingredientes para <strong style={{ color: 'white' }}>una</strong> unidad. Las variantes x2 y x3 se multiplican automáticamente.
              </div>
              ...
              {totalCost > 0 && (
                <div style={{ background: 'rgba(232,184,75,0.06)', border: '1px solid rgba(232,184,75,0.2)', borderRadius: 10, padding: 14, marginTop: 16 }}>
                  <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--gray)', marginBottom: 10, textTransform: 'uppercase' }}>Preview de costos (x1)</div>
          ...
          {wizardStep === 3 && (
            <>
              ...
              <div style={{ color: 'var(--gray)', fontSize: '0.8rem', marginBottom: 20 }}>
                Precio sugerido basado en tu escandallo: <strong style={{ color: 'var(--gold)' }}>{fmt(suggestedPrice)}</strong> (x1)
              </div>
              {['x1','x2','x3'].map((v, i) => {
                ...
                return (
                  <div key={v} style={{ background: '#111', border: '1px solid var(--border)', borderRadius: 10, padding: 14, marginBottom: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                      <div>
                        <span style={{ fontFamily: 'Bebas Neue', fontSize: '1.2rem', color: 'var(--gold)' }}>{v.toUpperCase()}</span>
                    ...
                    return (
                      <>
                        ...
                        {isExpanded && p.recipe?.ingredients?.length > 0 && (
                          <tr key={`${p._id}-recipe`}>
                            <td colSpan={10} style={{ background: 'rgba(232,184,75,0.04)', padding: '12px 20px' }}>
                              <div style={{ fontSize: '0.8rem', color: 'var(--gray)', fontWeight: 600, marginBottom: 8 }}>📋 FICHA TÉCNICA</div>
                              ...
                              <div style={{ marginTop: 8, fontSize: '0.78rem', color: 'var(--gray)' }}>
                                Costo ingredientes: <strong style={{ color: 'white' }}>{fmt(p.ingredientCost)}</strong>
                                &nbsp;+&nbsp; Indirectos ({totalIndirectPct}%): <strong style={{ color: 'white' }}>{fmt(p.indirectCost)}</strong>
                                &nbsp;=&nbsp; <strong style={{ color: 'var(--gold)' }}>Costo total: {fmt(p.totalCost)}</strong>
                              </div>
                            </td>
                          </tr>
                        )}
